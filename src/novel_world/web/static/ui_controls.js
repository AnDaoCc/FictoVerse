/**
 * 全站统一 UI 控件：原生 select → 主题一致的下拉面板（展开时 portal 到 body，避免被 .main 裁剪）
 */
(function () {
  ...
  const buildMenu = (select, menu, trigger, valueEl) => {
    ...
    const pick = (opt) => {
      ...
      if (select.dataset.navigate === "1" && select.value) {
  ...
  const enhanceSelect = (select) => {
    ...
    if (
      select.closest(
        ".chat-sidebar, .sidebar-panel, .compact-form, .model-picker, .group-controls, .rel-row, .card, .lore-form"
      )
    ) {
